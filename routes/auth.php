...
Route::get('/register/{ref_id?}/{plan_id?}/{lang?}', [RegisteredUserController::class, 'showRegistrationForm'])
    ->middleware('guest')
    ->name('register');
...
Route::get('/verify-email/{lang?}', [EmailVerificationNotificationController::class, 'showVerifyForm'])
    ->middleware('auth')
    ->name('verification.notice');
...
Route::get('/mobile-login', function (Request $request) {
    ...
    // 🔐 Resolve Sanctum token (CORRECT WAY)
    ...
    // ✅ Login user into WEB session
    Auth::guard('web')->login($accessToken->tokenable, true);
