...
Route::group(['middleware' => ['verified']], function () {
    ...
    Route::get('attendanceemployee/visits', [AttendanceEmployeeController::class, 'showVisits'])->name('attendanceemployee.visits')->middleware(
    [
        'auth',
        'XSS',
    ]
    ...
    Route::get(
    'attendance/missed',
    [App\Http\Controllers\AttendanceEmployeeController::class, 'missedAttendance']
)->name('attendanceemployee.missedattendance')
  ->middleware(['auth']);
    
    Route::post('/salary-import', [ImportController::class, 'saveSalaryImport'])->name('salary.import.save');
    ...
    Route::post('transferbalance/getaccount', [TransferBalanceController::class, 'getAccounts'])->name('transferbalance.getaccounts')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('messages/clear', [MessagesController::class, 'mark_all_messages_read'])->name('message.clear');
    
    // ====== CHATIFY ROUTES ======
    Route::group(['middleware' => ['auth', 'XSS']], function () {
    Route::get('/chats', [MessagesController::class, 'index'])->name('chats');
    Route::get('/chats/{id}', [MessagesController::class, 'show'])->name('chats.show');
    Route::post('/messages/send', [MessagesController::class, 'store'])->name('messages.store');
    Route::post('/messages/delete/{id}', [MessagesController::class, 'delete'])->name('messages.delete');
    Route::post('/messages/update/{id}', [MessagesController::class, 'update'])->name('messages.update');
    Route::get('/messages/get/{id}', [MessagesController::class, 'getMessages'])->name('messages.get');
    Route::post('/favorites/{id}', [MessagesController::class, 'favorite'])->name('favorites.toggle');
    });
    ...
    Route::post('google2f/', [Google2faController::class, 'index'])->name('google2fa.index')->middleware('auth');
    Route::post('google2f/enable', [Google2faController::class, 'enable'])->name('google2fa.enable')->middleware('auth');
    Route::post('google2f/disable', [Google2faController::class, 'disable'])->name('google2fa.disable')->middleware('auth');
