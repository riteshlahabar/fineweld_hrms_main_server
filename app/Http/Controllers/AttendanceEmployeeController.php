...
class AttendanceEmployeeController extends Controller
{
    public function index(Request $request)
    {
        if (\Auth::user()->can('Manage Attendance')) {
            ...
            if (\Auth::user()->type == 'employee') {
                ...
                $attendanceEmployee = $attendanceEmployee->latest('date')->get();
    ...
    public function bulkAttendance(Request $request)
    {
        if (\Auth::user()->can('Create Attendance')) {
            ...
            if (!empty($request->branch) && !empty($request->department) && $request->department >= 0) {
                $employees = Employee::where('created_by', \Auth::user()->creatorId())->where('branch_id', $request->branch)->where('department_id', $request->department)->
    ...
    /**
 * Show client visits for employee on specific date
 */
/**
 * Show client visits for employee on specific date
 */
public function showVisits(Request $request)
{
    ...
    // Get client visits for this employee on this date
    ...
    return view('attendance.visit', compact('employee', 'visits', 'breaks', 'date'));
}
    ...
    // ⬇️ Put this inside AttendanceEmployeeController (e.g., after bulkAttendance method)
public function missedAttendance(Request $request)
{
    if (\Auth::user()->can('Manage Attendance')) {
        ...
        $end   = date('Y-m-t', strtotime('01-' . $month . '-' . $year));

        $employees = Employee::where('created_by', \Auth::user()->creatorId())->pluck('id');

        $report = AttendanceEmployee::select(
                'attendance_employees.employee_id',
                'employees.name as employee_name',
                \DB::raw("SUM(CASE 
                    WHEN clock_in IS NULL 
                    OR clock_in = '' 
                    OR clock_in = '00:00:00' 
                    OR clock_out IS NULL 
                    OR clock_out = '' 
                    OR clock_out = '00:00:00' 
                    THEN 1 ELSE 0 END) AS missed"),
                \DB::raw("SUM(CASE 
                    WHEN clock_in IS NOT NULL 
                    AND clock_in != '' 
                    AND clock_in != '00:00:00' 
                    AND clock_out IS NOT NULL 
                    AND clock_out != '' 
                    AND clock_out != '00:00:00' 
                    THEN 1 ELSE 0 END) AS punched")
            )
            ->leftJoin('employees', 'employees.id', '=', 'attendance_employees.employee_id')
            ->whereIn('attendance_employees.employee_id', $employees)
            ->whereBetween('attendance_employees.date', [$start, $end])
            ->groupBy('attendance_employees.employee_id', 'employees.name')
            ->get();

        return view('attendance.missed', compact('report'));
    }
